...
#[derive(Debug, Parser)]
#[clap(author, version, about("Binary file to test select performance with page index"), long_about = None)]
struct Args {
    #[clap(short, long, help("Path to a parquet file, or - for stdin"))]
    file_name: String,
    #[clap(short('i'), long, help("Column idx to project"), value_delimiter = ',')]
    proj_col_idxs: Vec<usize>,
    #[clap(short('p'), long, help("position to select"))]
    pos: i64,
}
...
fn main() -> Result<()> {
    ...
    let proj_col_idxs: Vec<usize> = args.proj_col_idxs;
    let pos = args.pos;
    ...
    //     let offset_index = offset_indices.get(proj_col_idxs).ok_or_else(|| {
    ...
    //     let selections: VecDeque<RowSelector> = match &column_indices[proj_col_idxs] {
    //         Index::NONE => {
    //             println!("NO INDEX");
    ...
    for i in 0..metadata.num_row_groups() {
        ...
        if row_group_metadata.num_rows() + total_rows_skipped <= pos {
            total_rows_skipped += row_group_metadata.num_rows();
            continue;
        } else {
            found = true;
        }
        let row_group_reader = reader.get_row_group(i).unwrap();
        for j in &proj_col_idxs {
            // build selection via PageIndex here.
            let offset_index_col_chunk = &offset_index[i][*j];

            let mut selection: VecDeque<RowSelector> = vec![].into();
            for j in 0..offset_index_col_chunk.len() - 1 {
                let o = &offset_index_col_chunk[j];
                let row_cnt =
                    offset_index_col_chunk[j + 1].first_row_index - o.first_row_index;
                if o.first_row_index <= pos
                    && pos < offset_index_col_chunk[j + 1].first_row_index
                {
                    selection.push_back(RowSelector::select(row_cnt as usize));
                } else {
                    selection.push_back(RowSelector::skip(row_cnt as usize));
                }
            }
            if pos >= offset_index_col_chunk.last().unwrap().first_row_index {
                selection.push_back(RowSelector::select(
                    (row_group_metadata.num_rows()
                        - offset_index_col_chunk.last().unwrap().first_row_index)
                        as usize,
                ));
            } else {
                selection.push_back(RowSelector::skip(
                    (row_group_metadata.num_rows()
                        - offset_index_col_chunk.last().unwrap().first_row_index)
                        as usize,
                ));
                ...
                ColumnReader::Int64ColumnReader(ref mut typed_reader) => {
                    loop {
                        ...
                        while read_records < batch_size && !selection.is_empty() {
                            ...
                            if front.skip {
                                let skipped =
                                    match typed_reader.skip_records(front.row_count) {
                                        Ok(skipped) => skipped,
