...
fn main() -> Result<()> {
    ...
    for i in 0..metadata.num_row_groups() {
        ...
        if let Some(sbbf) = row_group_reader.get_column_bloom_filter(filter_col_idx) {
            if sbbf.check(&v1.parse::<i64>().unwrap()) {
                // present
                // println!("row group present: {}", i);
            } else {
                // println!("skip row group: {}", i);
                continue;
            }
        }
